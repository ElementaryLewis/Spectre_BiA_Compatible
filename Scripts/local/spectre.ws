...
function spectreIsQoLOption(optionName : name) : bool
{
	return (optionName == 'spectreSignInstantCast' || optionName == 'spectreLowStaminaSFX'
			|| optionName == 'spectreLowStaminaSFXVolume' || optionName == 'spectreLowStaminaSFXRate'
			|| optionName == 'spectreLowStaminaSFXDynamicVolume' || optionName == 'spectreLowStaminaSFXDynamicRate');
}
...
{
	if( FactsQuerySum("Spectre_Initialized_Settings") <= 0)
	{
		...
		theGame.GetInGameConfigWrapper().ApplyGroupPreset('spectreQOLOptions', 0);

		theGame.SaveUserSettings();

		FactsAdd("Spectre_Initialized_Settings", 1);
	}
}
