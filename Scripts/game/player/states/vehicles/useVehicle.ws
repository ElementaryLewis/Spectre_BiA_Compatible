...
statemachine class W3VehicleCombatManager extends CEntity
{
	...
	//modSpectre begin
	protected var ON_HORSEBACK_STAMINA_MULTIPLIER : float;
	default ON_HORSEBACK_STAMINA_MULTIPLIER = 3.0;
	//modSpectre end
...
state Null in W3VehicleCombatManager
{
	...
	event OnItemAction( action : SInputAction )
	{
		...
		//modSpectre begin
		if(rider == GetWitcherPlayer() && !thePlayer.HasStaminaToUseAction(ESAT_UsableItem))
		{
			if(IsPressed(action))
			{			
	...
	function StartAttackAction()
	{
		if ( rider.GetWeaponHolster().IsMeleeWeaponReady() )
		{
			...
			{
				//modSpectre begin
				if(rider == GetWitcherPlayer() && !thePlayer.HasStaminaToUseAction(ESAT_LightAttack,,,parent.ON_HORSEBACK_STAMINA_MULTIPLIER))
				{
					thePlayer.SoundEvent( "gui_ingame_low_stamina_warning" );
				}
				else
...
state SwordAttack in W3VehicleCombatManager
{
	...
	event OnAttack( action : SInputAction )
	{
		...
		{
			//modSpectre begin
			if(rider == GetWitcherPlayer())
			{
				thePlayer.DrainStamina(ESAT_LightAttack,,,,,parent.ON_HORSEBACK_STAMINA_MULTIPLIER);
			}
			//modSpectre end
			if( CanPerformAttack() )
				FinishAttack();
	...
	private function DealDamageToHostiles( entities : array<CGameplayEntity>, speed : float, baseDamage : float )
	{
		...
		var dmgInfos : array< SRawDamage >; //modSpectre
		var dmgSize, k : int; //modSpectre
		...
		for( i = 0 ; i < entities.Size() ; i += 1 )
		{
			...
			if( actor )
			{
				//actor.DrainStamina(ESAT_FixedValue, 100, 1); //modSpectre
				//theGame.witcherLog.AddMessage("Attacking from horse: " + actor.GetDisplayName()); //modSpectre
				...
				if(speed > 0)
				{
					dmgSize = damage.GetDTs( dmgInfos );
					for(k = 0; k < dmgSize; k += 1)
					{
						if(dmgInfos[k].dmgVal > 0)
						{
							damage.AddDamage(dmgInfos[k].dmgType, dmgInfos[k].dmgVal*ClampF(speed/4.0, 0.0f, 1.0f));
						}
					}
				}
				//modSpectre end
...
state CastSign in W3VehicleCombatManager
{
	...
	private var ON_HORSEBACK_AXII_COST : float; default ON_HORSEBACK_AXII_COST = 50; //modSpectre
	private var staminaSpent : float; //modSpectre
	...
	event OnEnterState( prevStateName : name )
	{
		...
		staminaSpent = -1; //modSpectre
		
		if( witcher && HasResourcesForAxii(ON_HORSEBACK_AXII_COST) ) //modSpectre
		{
			StartCastingSign();
		}
		else
	...
	//modSpectre begin
	function HasResourcesForAxii(cost : float) : bool
	{
		var ret : bool;
		ret = (thePlayer.GetStat(BCS_Stamina) >= cost);
		if(!ret && thePlayer.CanUseSkill(S_Perk_10))
		{
			ret = ((thePlayer.GetStat(BCS_Stamina) + thePlayer.GetStat(BCS_Focus)*thePlayer.GetStatMax(BCS_Stamina)/thePlayer.GetStatMax(BCS_Focus)) >= cost);
		}
		return ret;
	}
	
	function DrainResourcesForAxii(cost : float)
	{
		var delay, savedStamina : float;
		
		savedStamina = thePlayer.GetStat(BCS_Stamina);
		if(savedStamina > 0)
		{
			thePlayer.DrainStamina(ESAT_FixedValue, cost, 0.5);
		}
		if(cost > savedStamina && thePlayer.CanUseSkill(S_Perk_10))
		{
			thePlayer.DrainFocus((cost - savedStamina)*thePlayer.GetStatMax(BCS_Focus)/thePlayer.GetStatMax(BCS_Stamina));
		}
		staminaSpent += cost;
	}
	//modSpectre end
	...
	entry function StartCastingSign()
	{
		...
		staminaSpent = 0; //modSpectre
		
		while( true )
		{
			//SleepOneFrame();
			Sleep(0.1f); //modSpectre
			
			if( theInput.IsActionPressed( 'VehicleCastSign' ) && HasResourcesForAxii(ON_HORSEBACK_AXII_COST*0.01) )
			{
				ApplyEffectOnHorse( 0.5 ); //modSpectre
				DrainResourcesForAxii(ON_HORSEBACK_AXII_COST*0.01); //modSpectre				  
			}
			else
			{
				break;
	...
	event OnLeaveState( nexStateName : name )
	{
		...
		//if( staminaSpent >= 0 && staminaSpent < ON_HORSEBACK_AXII_COST ) //modSpectre
		//{
		//	DrainResourcesForAxii(ON_HORSEBACK_AXII_COST - staminaSpent);
		//}
